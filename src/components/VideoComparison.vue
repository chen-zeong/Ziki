<template>
  <div class="comparison-container overflow-hidden h-full">
    ...
    <Transition name="modal" appear>
      ...
      >
        ...
        >
          ...
          <!-- 全屏滑块 -->
          <div class="slider fullscreen-slider-line" @mousedown="startFullscreenDragging">
            <div class="slider-handle fullscreen-slider-handle">
              <svg class="w-5 h-5 text-orange-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
              </svg>
              <svg class="w-5 h-5 text-orange-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
    ...
    <!-- 帧选择器 -->
    <div v-if="props.videoPath" class="mb-6">
      <!-- 扁平线条帧选择器 -->
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6 overflow-hidden">
      ...
      <div class="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-lg overflow-hidden max-h-full">
        <div class="space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">视频编码</label>
            <CustomSelect 
          ...
          <div>
            <div class="flex justify-between items-center mb-2">
              ...
              <button
                type="button"
                class="px-3 py-1 text-sm font-medium transition-all duration-200 rounded-md"
                :class="isCustomResolution ? 'bg-amber-500 text-white shadow-sm' : 'text-gray-600 dark:text-gray-300 hover:text-amber-600 dark:hover:text-amber-400'"
                @click="toggleCustomResolution"
              >
                自定义
              </button>
            </div>
            <div v-if="isCustomResolution" class="mb-2 grid grid-cols-2 gap-2">
              <CustomNumberInput
    ...
    <button 
      class="w-full bg-amber-500 text-white font-bold py-3 px-4 rounded-lg hover:bg-amber-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500 dark:focus:ring-offset-gray-900 transition-all transform hover:scale-105 mt-4"
      :disabled="props.isProcessing"
      @click="startCompression"
    >
      {{ props.isProcessing ? '压缩中...' : '开始压缩' }}
    </button>
  </div>
</template>
...
const crfColorClass = computed(() => {
  const crf = settings.crfValue || 23;
  if (crf >= 18 && crf <= 28) {
...
const startCompression = () => {
  const compressionSettings: CompressionSettings = {
    ...settings,
    customResolution: settings.resolution === 'custom' ? customResolution.value : undefined,
    bitrate: settings.qualityType === 'bitrate' ? `${bitrateValue.value}k` : undefined
  };
